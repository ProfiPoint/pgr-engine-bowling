...
namespace copakond {
    ...
    void Input::keyboardInputEvent(unsigned char key, int x, int y) {
        keys_map[std::tolower(key)] = true;
        keys_map[std::toupper(key)] = true;
    }
    ...
    void Input::keyboardUpInputEvent(unsigned char key, int x, int y) {
        keys_map[std::tolower(key)] = false;
        keys_map[std::toupper(key)] = false;
    }
    ...
    void Input::keyInput(float deltaTime) {
        if (keys_map['w']) camera.processKeyboard(FRONT, deltaTime);
        if (keys_map['s']) camera.processKeyboard(BACK, deltaTime);
        if (keys_map['a']) camera.processKeyboard(LEFT, deltaTime);
        if (keys_map['d']) camera.processKeyboard(RIGHT, deltaTime);
